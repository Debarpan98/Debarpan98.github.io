...
<div style="text-align: justify">While using modern cameras for retinal image acquisition, there often occurs some reflections from the retinal surface that causes luminosity and contrast variations. These irregularities in illumination might introduce distortions in the acquired image thereby distorting the anatomical structures. So, the illumination correction proposed by Savelli et. al, which draws a connection between shadows and haze, is used to reduce these illumination defects in this project. Since the performance of this method had only been tested on older retinal vessel segmentation algorithms, we attempt to utilize the processed by this method to train modern vessel segmentation networks like UNet, Laddernet and DU-Net and compare their performances.</div>
